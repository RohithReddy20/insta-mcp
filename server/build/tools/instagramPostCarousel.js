...
exports.instagramPostCarouselOutputSchema = exports.instagramPostCarouselInputSchema = void 0;
exports.postCarouselToInstagram = postCarouselToInstagram;
...
const carouselMediaItemSchema = zod_1.z.object({
    type: zod_1.z.enum(["IMAGE", "VIDEO"]).describe("Type of media: IMAGE or VIDEO."),
    url: zod_1.z
        .string()
        .url()
...
exports.instagramPostCarouselInputSchema = zod_1.z.object({
    igUserId: zod_1.z
        .string()
    ...
    caption: zod_1.z.string().optional().describe("The caption for the carousel post."),
    userAccessToken: zod_1.z
        .string()
        .describe("The access token of the Instagram user."),
});
// Define the schema for the tool output
exports.instagramPostCarouselOutputSchema = zod_1.z.object({
    postId: zod_1.z.string().describe("The ID of the created Instagram carousel post."),
    status: zod_1.z.string().describe("Status of the carousel post creation"),
});
// Function to post a carousel to Instagram, adapted from instagram.service.ts
async function postCarouselToInstagram(input) {
    ...
    try {
        ...
        for (const item of mediaItems) {
            ...
            const itemResponse = await (0, node_fetch_1.default)(itemContainerUrl, {
                method: "POST",
            ...
            const { id: childContainerId } = itemResponseData;
            if (!childContainerId)
                throw new InstagramApiError(`Media container ID not found for item ${item.url}.`, InstagramErrorType.UNKNOWN_ERROR);
        ...
        let response = await (0, node_fetch_1.default)(carouselContainerUrl, {
            method: "POST",
            headers: { "Content-Type": "application/x-www-form-urlencoded" },
        ...
        const { id: mainContainerId } = responseData;
        if (!mainContainerId)
            throw new InstagramApiError("Main carousel container ID not found.", InstagramErrorType.UNKNOWN_ERROR);
        ...
        if (!mediaId)
            throw new InstagramApiError("Post ID not found in carousel publish response.", InstagramErrorType.UNKNOWN_ERROR);
